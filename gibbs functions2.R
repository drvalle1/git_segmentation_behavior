...
get.summary.stats2=function(breakpt,dat,max.SL,max.TA){  #include Step Length Autocorr to original model
  breakpt1=c(0,breakpt,Inf)
  n=length(breakpt1)
...
#---------------------------------------------
log.marg.likel2=function(alpha,summary.stats,bern.a,bern.b,max.SL,max.TA){
  #get ratio for SL
  lnum=rowSums(lgamma(alpha+summary.stats$res.SL))
  lden=lgamma(max.SL*alpha+rowSums(summary.stats$res.SL))
  p2=sum(lnum)-sum(lden)
...
#---------------------------------------------
samp.move2=function(breakpt,max.time,dat,alpha,bern.a,bern.b,max.SL,max.TA){
  breakpt.old=breakpt
  p=length(breakpt)
  rand1=runif(1)	
  p0=1
  ...
  #get sufficient statistics
  stats.old=get.summary.stats2(breakpt=breakpt.old,dat=dat,max.SL=max.SL,max.TA=max.TA)
  stats.new=get.summary.stats2(breakpt=breakpt.new,dat=dat,max.SL=max.SL,max.TA=max.TA)
  
  #get marginal loglikel
  pold=log.marg.likel2(alpha=alpha,summary.stats=stats.old,bern.a=bern.a,bern.b=bern.b,max.SL=max.SL,max.TA=max.TA)
  pnew=log.marg.likel2(alpha=alpha,summary.stats=stats.new,bern.a=bern.a,bern.b=bern.b,max.SL=max.SL,max.TA=max.TA)+log(p0)
  prob=exp(pnew-pold)
  rand2=runif(1)
